...
from rich import print  # Import rich for rich-formatted console output.
from groq import Groq  # Import Groq API functionalities.
...
classes = ["zCubwf", "hgKElc", "LTKOO sYricc", "Z0LcW", "gsrt vk_bk FzvWSb YwPhnf", "pclqee", "tw-Data-text tw-text-small tw-ta",
           "lJz6rdc", "OSrU8d LTKOO", "vLzy6d", "webanswers-webanswers_table__webanswers-table", "dDoNo ikb4Bb gsrt", "sXLaOe",
           "Lwfkfke", "VQF4g", "qV3wPe", "kn0rdescc", "SPZz6b"]
...
def Content(Topic):
    ...
    # Nested function to genetrate content using AI chatbot.
    def ContentWriterAI(prompt):
        messages.append({"role": "user", "content": prompt})  # Add user prompt to messages.
...
def OpenApp(app, sess=requests.session()):
    try:
        appopen(app, match_closest=True, output=True, throw_error=True)  # Try to open local app
        return True

    except:
        # If not an installed app, fallback to web search
        if app.lower() == "facebook":  # Special case
        ...
        if links:
            ...
            print(f"[red]No search results found for {app}[/red]")
...
tools = [
    {
        ...
        "function": {
            "name": "open_app",
            "description": "Open a local app or website",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "Name of the app or site"}
                },
                "required": ["name"]
            }
        }
    ...
    # (You can add GoogleSearch, Content, System in the same way)
]

def LLMwithTools(user_prompt):
    ...
    if hasattr(msg, "tool_calls") and msg.tool_calls:
        for call in msg.tool_calls:
            ...
            elif call.function.name == "close_app":
                return CloseApp(args["name"])
            elif call.function.name == "play_youtube":
                return PlayYoutube(args["query"])
    else:
        return msg.content
